...
37. **Duplicate-punch guard (5s) + hidden audit rows on employee history** — Fixes for the "two morning punch records" complaint (2026-07-17). (a) **Server guard**: `punch()` rejects with 400 (`ValueError`, "Duplicate punch …短時間內重複打卡") when any *non-overridden* log for the same employee/day lies within `DUPLICATE_PUNCH_WINDOW_SECONDS = 5` (absolute difference vs naive `now()`, consistent with #34) — placed between the weekend gate and the geolocation call. Overridden logs never trip it. Known accepted edge: a same-day pre-filled override log within ±5s of the live punch also blocks (harmless; pre-fill targets future days). (b) **Client lock**: the punch page adds a synchronous `submitLockRef` (`useRef`) guarding both submit paths (direct + geolocation-effect), because `isSubmitting` state updates too late to stop a double-tap. (c) **Employee attendance history (`/attendance`) hides `is_overridden` rows** (`visibleLogs` filter; 已覆寫 badge JSX removed, i18n key kept). Rationale: bulk override (#20/#23) re-creates a day's punches as 0,0/"override"/OFFICE logs and tags the original GPS punch 已覆寫, so employees saw every touched morning "duplicated". Event views for managers/HR (team page) still show the full audit trail. Known narrow gap: a day whose only logs came from the legacy single manager-override endpoint (creates `is_overridden=True` directly) now renders nothing on this page — status still visible in monthly-override and reports.

38. **Bulk-override clear semantics: explicit null clears, omitted key preserves** — Fixes "cleared a mistaken 假別/備註 on monthly-override, saved, reload shows old values" (2026-07-30). The old `PUT /api/attendance/override-bulk` treated `leave_type`/`remark`/`overtime_hours = None` as "not provided", skipped the summary-stamping block, and `generate_daily_summary` round-tripped the previous values back — the API literally could not express "clear". Now the router passes each entry via `model_dump(exclude_unset=True)` and the service keys off **key presence**: a key explicitly present as JSON `null` clears the stored value; an omitted key leaves it alone (backward-compatible with clients that don't send the field). The frontend needed no change — it already sends every field of a changed row, with cleared inputs as `null` (`JSON.stringify` keeps nulls). Two related behaviors: (a) clearing the leave on a row with no punches stamps an `ABSENT` placeholder status (instead of the old row's status) because `generate_daily_summary` returns `None` for a punchless/leaveless day and would otherwise strand a stale `LEAVE` badge; (b) an entry whose only effect would be "clear values on a day that has no summary row" creates nothing. This also fixed a latent bug where a raw-API entry that set only `remark` would silently null out a stored `overtime_hours` (the stamping block used to pass all three values unconditionally). Tests: `tests/integration/test_bulk_override_clear.py`.

39. **`late_leave_reason` + confirm-only punch dialog + dual submission gate + HRM export + export exclusion + monthly-override lock + `/me` shift times + schedule-confirmation banner** — Phase 14 wrap-up patch (2026-08-03). (a) New `late_leave_reason` column (`daily_attendance_summaries`, `Literal["ASSIGNED_OVERTIME", "PERSONAL"]`, max 30) round-trips through live punch, bulk-override, and `generate_daily_summary`/`generate_all_summaries` exactly like `leave_type`/`remark`/`overtime_hours` (same `_UNSET`-sentinel and key-presence-clears semantics as #38). **Required** whenever a day is LATE/EARLY_LEAVE/LATE_AND_EARLY_LEAVE with no reason set — except a day that already has a `leave_type` is exempt *unless* that leave type is in the configurable `late_reason_required_leave_types` list (`system_config`, default `["出差"]`, since a business-trip day can still start late and needs explaining). The punch page shows a **confirm-only** dialog (no free text — pick ASSIGNED_OVERTIME or PERSONAL, defaulting to PERSONAL) when the punch lands after a non-exempt shift-end on a working day (`day_kind`-aware, mirroring #31f); this required the punch page to probe `/api/config/workdays` on **every** load (previously the calendar probe only ran on Saturdays, since #31f's weekday fallback already covered the other six days) — that change also surfaced a latent UTC-rollback bug (`toISOString()` before 08:00 Taipei time matched the wrong calendar day, e.g. Monday 07:45 wrongly resolving to Sunday/`REGULAR_LEAVE`), fixed by switching the probe's date key to the existing `localDateString()` helper (same UTC-rollback class of bug as the frontend date-key fix predating this branch). Enforcement is a **dual gate**: the monthly-override page hard-blocks 本月送單 client-side (scans all rows, opens a modal listing offending dates) and, independently, `POST /api/monthly-submissions` returns 400 with structured detail `{"code": "late_reason_missing", "dates": [...]}` — the frontend also opens the same modal from those dates when the local scan misses a backend-only case (`ApiError.raw` surfaces the structured detail). (b) **HRM export** (`GET /api/reports/export?format=hrm`) emits one row **per punch** (not per day) in the format the company's separate HRM/payroll system ingests: 考勤機ID left blank, source label always fixed `"APP打卡"` regardless of how the punch was recorded (live, override, or NFC gap-fill), dates unpadded (`M/D` not `MM/DD`). (c) **`export_excluded_emp_ids`** (`system_config`, default `["HR01", "ADMIN"]`) hides listed emp_ids from **all four** export formats (CSV/JSON/Excel/HRM) and **wins over an explicit `emp_id` filter** — requesting an excluded employee's export by ID still returns nothing, unlike the terminated-employee override in #26. (d) **`monthly_override_lock`** (`system_config`, global boolean, default off) blocks `PUT /api/attendance/override-bulk` and `POST /api/monthly-submissions` for EMPLOYEE/MANAGER once set; HR/ADMIN are always exempt. Managed via `GET /api/config/override-lock` (any auth) / `PUT` (HR+), surfaced in an admin lock/release section; the monthly-override page renders read-only (no inputs, disabled Save/本月送單) with a red `role="alert"` banner (`data-testid="override-lock-banner"`) for locked-out roles. (e) `GET /api/auth/me` now additionally returns the employee's name and `shift_start_time`/`shift_end_time` (as `"HH:MM"` strings) so the punch page can compute its late-dialog trigger without a second round-trip. (f) **Schedule-confirmation banner** — an amber `role="note"` notice (`data-testid="schedule-notice"`) directly below the existing 24-hour time-format hint on the monthly-override page, visible to every role, stating the month's schedule is deemed accepted absent timely feedback to the supervisor/HR (i18n `monthlyOverride.scheduleNotice`, zh/en).
...
| 14 | UX Enhancements (late-leave reason, HRM export, override lock, schedule notice — see #39) | — | Done |
| 15 | Org Reporting Hierarchy (reports_to, rank, subtree-scoped authority, org-scoping toggle) | — | Done |

**Current test count: 479 backend (91.26% coverage) + 186 frontend passing (187 total, 1 pre-existing flake) = 665 passing + 33 Playwright E2E stubs** (the flake is in `monthly-override/page.test.tsx` — a `waitFor` timeout, unrelated to current work)
