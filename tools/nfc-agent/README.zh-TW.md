...
2. 設定 API 金鑰為機器環境變數（必須跟後端的 `NFC_IMPORT_API_KEY` 一致）：

   ```powershell
   [Environment]::SetEnvironmentVariable("NFC_IMPORT_API_KEY", "<你的密鑰>", "Machine")
   ```

3. 註冊每日排程工作（每天 00:20 執行，晚於門禁系統 00:10 的匯出時間）：
...
- 參數皆可覆寫，例如：`push-nfc.ps1 -Folder "D:\door" -ApiUrl "https://…/api/nfc/import"`。
- 門禁主機的 DHCP IP 無關緊要——本程式只會對外發送請求（outbound），不需要對內開放任何連接埠。
- 傳送的是原始 CP950 編碼的檔案內容（bytes），解碼工作由後端負責。
