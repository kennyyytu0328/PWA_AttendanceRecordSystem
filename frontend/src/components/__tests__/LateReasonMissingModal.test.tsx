...
const DATES: readonly string[] = ["2026-07-22", "2026-07-23"];
...
describe("LateReasonMissingModal", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("renders nothing when open=false", () => {
    const { container } = render(
      <LateReasonMissingModal open={false} dates={[...DATES]} onClose={vi.fn()} />,
    );
  ...
  it("renders the title and lists every offending date", () => {
    ...
    expect(
      screen.getByText("monthlyOverride.lateReasonMissingTitle"),
    ).toBeInTheDocument();
