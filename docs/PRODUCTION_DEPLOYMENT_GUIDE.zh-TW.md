...
### 4.1 確認容器時區（Asia/Taipei）

**這是必做、非選用。** 系統以 naive 的 `datetime.now()` 記錄現場打卡,並以台灣牆鐘時間判定遲到/早退,因此容器時鐘**必須**是台灣時間。容器預設為 UTC,會讓每筆現場打卡默默偏移 −8 小時(08:53 被存成、顯示成 00:53)。三個服務皆已於 `docker-compose.prod.yml` 透過 `TZ: Asia/Taipei` 釘住,且 backend 的 slim image 已安裝 `tzdata`(否則 glibc 會忽略 `TZ` 並退回 UTC)。

```bash
docker compose -f docker-compose.prod.yml exec backend date
# → 必須顯示 CST 與 +0800,例如 "Tue Jun 30 09:05:19 CST 2026"
# 若顯示 UTC,代表重建時沒裝到 tzdata — 請以 --build 重新建置。
```

> **既有資料不會被回溯修正。** 設定時區只影響重啟**之後**的打卡。重啟前在 UTC 下記錄的現場打卡仍偏移 −8 小時,須透過「月度補登」頁面逐日修正;底層 raw punch 時間改對後,當日 summary(及 LATE/EARLY_LEAVE 狀態)會在下次讀取時自動重算修復。
...
- [ ] `docker compose exec backend date` 顯示 **CST / +0800**(台灣時間),非 UTC — 見 §4.1
...
| 現場打卡時間早 8 小時(08:53 顯示成 00:53);遲到狀態錯誤 | 容器跑在 UTC 而非台灣時間(缺 `TZ` 或 `tzdata`) | 確認 `docker compose exec backend date` 顯示 `+0800`;若否,確保已設 `TZ: Asia/Taipei` 並重建以安裝 `tzdata`(`up -d --build backend`)。見 §4.1。既有 −8h 紀錄須透過「月度補登」修正。 |
